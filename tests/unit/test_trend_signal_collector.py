...
class TestCollectTopTrends:
    def test_creates_knowledge_items(self, collector, kb):
        ...
        with patch("nexus.data.trend_signal_collector.urlopen",
    ...
    def test_returns_combined_stats(self, collector, kb):
        ...
        def side_effect(req, timeout=5):
            ...
            if "trends/top" in url:
                return _mock_urlopen_response(trends_data)
            if "signals" in url:
                return _mock_urlopen_response(signals_data)
            return _mock_urlopen_response(lifecycle_data)

        with patch("nexus.data.trend_signal_collector.urlopen", side_effect=side_effect):
