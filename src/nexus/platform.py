...
class PlatformConfig:
    ...
    default_model: str = "ollama-csuite-model"
...
class NexusPlatform:
    ...
    async def _init_component(self, status_key: str, init_fn) -> None:
        """Shared helper: run *init_fn*, log failures, update ``_status``."""
        try:
            init_fn()
            self._status[status_key] = True
        except ImportError as exc:
            logger.warning("Failed to import %s component: %s", status_key, exc)
            self._status[status_key] = False
        except Exception as exc:
            logger.error(
                "Failed to initialize %s component: %s: %s",
                status_key, type(exc).__name__, exc,
            )
            self._status[status_key] = False
    ...
    async def _ensure_llm_component(self) -> None:
        ...
        if self._llm is None:
            def _init():
                ...
                self._llm = NexusLLM()
            await self._init_component("llm", _init)
        else:
    ...
    async def _ensure_ensemble_component(self) -> None:
        ...
        if self._ensemble is None:
            ...
            await self._init_component("ensemble", _init)
        else:
    ...
    async def _ensure_cognitive_components(self) -> None:
        ...
        await self._init_component("cog_eng", _init)
    ...
    async def _ensure_codegen_component(self) -> None:
        ...
        await self._init_component("codegen", _init)
    ...
    async def _ensure_expert_components(self) -> None:
        ...
        await self._init_component("experts", _init)
    ...
    async def _ensure_monitoring_components(self) -> None:
        ...
        await self._init_component("observatory", _init)
    ...
    async def _ensure_resource_discovery_core(self) -> None:
        ...
        await self._init_component("discovery", _init)
    ...
    async def _ensure_insights_components(self) -> None:
        ...
        await self._init_component("insights", _init)
    ...
    async def _ensure_discovery_components(self) -> None:
        ...
        try:
            ...
            logger.warning("Failed to import discovery components: %s", exc)
            self._status["discovery"] = False
        except Exception as exc:
            logger.error("Failed to initialize discovery: %s: %s", type(exc).__name__, exc)
            self._status["discovery"] = False
