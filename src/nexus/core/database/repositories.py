...
def _safe_commit(session: Session, context: str) -> None:
    """Commit the session, rolling back on failure."""
    try:
        session.commit()
    except SQLAlchemyError as exc:
        session.rollback()
        logger.error("Transaction failed (%s): %s: %s", context, type(exc).__name__, exc)
        raise
...
class UserRepository:
    ...
    def create(self, user_id: str, username: str, email: str, role: str = "user") -> UserModel:
        ...
        _safe_commit(self.session, f"create user {username}")
    ...
    def update(self, user: UserModel):
        """Update user."""
        _safe_commit(self.session, f"update user {user.user_id}")
        self.session.refresh(user)
    ...
    def delete(self, user_id: str):
        ...
        if user:
            ...
            _safe_commit(self.session, f"delete user {user_id}")
...
class APIKeyRepository:
    ...
    def create(
        ...
        _safe_commit(self.session, f"create API key {name}")
        self.session.refresh(api_key)
        logger.info("Created API key: %s (%s) for user %s", name, key_id, user_id)
        return api_key
    ...
    def update_last_used(self, key_id: str):
        ...
        if api_key:
            ...
            _safe_commit(self.session, f"update_last_used {key_id}")
    ...
    def revoke(self, key_id: str):
        ...
        if api_key:
            ...
            _safe_commit(self.session, f"revoke API key {key_id}")
    ...
    def delete(self, key_id: str):
        ...
        if api_key:
            ...
            _safe_commit(self.session, f"delete API key {key_id}")
...
class CostEntryRepository:
    ...
    def create(
        ...
        _safe_commit(self.session, "create cost entry")
        return cost_entry
...
class UsageEntryRepository:
    ...
    def create(
        ...
        _safe_commit(self.session, f"create usage entry for {endpoint}")
        return usage_entry
...
class StrategyPerformanceRepository:
    ...
    def get_or_create(
        ...
        if not perf:
            ...
            _safe_commit(self.session, f"create strategy perf {strategy_name}/{model_name}")
    ...
    def update(
        ...
        _safe_commit(self.session, f"update strategy perf {strategy_name}/{model_name}")
...
class RateLimitRepository:
    ...
    def record(self, key_id: str, timestamp: Optional[datetime] = None):
        ...
        _safe_commit(self.session, f"record rate limit for {key_id}")
    ...
    def cleanup_old_entries(self, days: int = 7):
        ...
        _safe_commit(self.session, f"cleanup rate limit entries older than {days} days")
        logger.info("Cleaned up rate limit entries older than %d days", days)
