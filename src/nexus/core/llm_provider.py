...
def _is_rate_limit_error(exc: Exception) -> bool:
    msg = str(exc).lower()
    return any(x in msg for x in ("429", "rate_limit", "rate limit", "quota", "resource_exhausted", "too many requests"))
...
class RoutingConfig:
    ...
    complexity_models: Dict[TaskComplexity, str] = field(default_factory=lambda: {
        TaskComplexity.SIMPLE: "runpod-csuite-merged",
        TaskComplexity.MODERATE: "runpod-csuite-merged",
        TaskComplexity.COMPLEX: "runpod-csuite-merged",
        TaskComplexity.CRITICAL: "runpod-csuite-merged",
    })
    ...
    # RunPod pod first, then local Ollama, then free cloud (OpenRouter)
    fallback_chain: List[str] = field(default_factory=lambda: [
        "runpod-csuite-merged",
        "ollama-csuite-model",
        "ollama-qwen-7b",
        "openrouter-qwen3-coder",
        "openrouter-qwen3-80b",
        "claude-max-sonnet",
    ])
...
class NexusLLM:
    ...
    def _get_model_for_task(self, task_type: str) -> str:
        ...
        return model or "ollama-qwen-7b"
        ...
        for preset in fallback_chain:
            ...
            try:
                ...
                if _is_rate_limit_error(e):
                    ...
                    logger.info("Rate limit hit on backend %s, retrying after 2s...", preset)
                    ...
                    try:
                        ...
                        logger.info("Rate limit retry failed on %s, falling through: %s", preset, retry_err)
